...
if (str_starts_with($id, "mb")) {
    $sql = "SELECT height_m, weight_kg, fitness_goal, injury_history, nutration_preference FROM member WHERE member_id='$id'";
    ...
    if ($result->num_rows > 0) {
        ...
        $injury = $row['injury_history'];
        $nutration_type = $row['nutration_preference'];
    }
}
else{
    echo "<script>alert('Your are not member. So your preference is not stored. So give us the information'); window.location.href='../smart_recommendation/reco_input.html';</script>";
}
